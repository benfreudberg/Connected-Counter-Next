...
int enterBootloader(pin_t boot0Pin, pin_t resetPin, uint32_t options) {
  ...
  // ben - I changed this so that it repeatedly sends the start command until it gets an ACK
  // see https://github.com/particle-iot/STM32_Flash/issues/1
  for (int i = 0; i < 100; i++) {
    // Send the bootloader start command
    Serial1.write(ENTER_BOOTLOADER);
    // Wait to get acknowledgement
    waitFor(Serial1.available, 10);
